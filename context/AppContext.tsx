...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  */
  ...
  const setActiveRide = (ride: RideRequest | null) => {
      ...
      if (ride) {
          safeStorage.setItem(STORAGE_KEYS.ACTIVE_RIDE_ID, ride.id);
      } else {
          safeStorage.removeItem(STORAGE_KEYS.ACTIVE_RIDE_ID);
      }
  };

  const [serviceType, setServiceTypeState] = useState<'ride' | 'errand' | 'move'>(() => {
     const saved = safeStorage.getItem(STORAGE_KEYS.SERVICE_TYPE);
  ...
  useEffect(() => {
      const savedRideId = safeStorage.getItem(STORAGE_KEYS.ACTIVE_RIDE_ID);
      ...
      const storedSession = safeStorage.getItem(STORAGE_KEYS.USER_SESSION);
      if (storedSession && !user) {
          try {
              ...
              safeStorage.removeItem(STORAGE_KEYS.USER_SESSION);
          }
      }
  }, []);
  ...
  useEffect(() => {
    if (!isSupabaseConfigured) {
        console.warn("Supabase not configured. Auth listeners skipped.");
        return;
    }
  ...
  useEffect(() => {
    safeStorage.setItem(STORAGE_KEYS.VIEW, view);
  }, [view]);

  useEffect(() => {
    safeStorage.setItem(STORAGE_KEYS.SERVICE_TYPE, serviceType);
  }, [serviceType]);
  ...
  const signInWithEmail = async (email: string, password: string) => {
      if (email.toLowerCase().includes('demo')) {
          ...
          setUser(mockUser);
          safeStorage.setItem(STORAGE_KEYS.USER_SESSION, JSON.stringify(mockUser));
      ...
      try {
          ...
          if (msg.includes('fetch') || msg.includes('network') || msg.includes('connection')) {
               const stored = safeStorage.getItem(STORAGE_KEYS.USER_SESSION);
               if (stored) {
  ...
  const signUpWithEmail = async (email: string, password: string, phone: string, role: UserRole) => {
      try {
          ...
          if (msg.includes('fetch') || msg.includes('network') || msg.includes('connection')) {
              ...
              setUser(localUser);
              safeStorage.setItem(STORAGE_KEYS.USER_SESSION, JSON.stringify(localUser));
              setRegistrationRole(role);
              return; 
          }
          throw err;
  ...
  const updateUserProfile = async (profileData: any) => {
      try {
        ...
        if (authUser) {
            ...
            setUser(updatedUser);
            safeStorage.setItem(STORAGE_KEYS.USER_SESSION, JSON.stringify(updatedUser));
        } else {
  ...
  const updateUser = (updates: Partial<User>) => {
      if (user) { 
          ...
          setUser(newUser);
          if (safeStorage.getItem(STORAGE_KEYS.USER_SESSION)) {
              safeStorage.setItem(STORAGE_KEYS.USER_SESSION, JSON.stringify(newUser));
          }
      }
  };
  ...
  console.log("AppProvider: Render logic complete, returning Provider");
  return (
