...
import { RideRequest, RideStatus, ErrandItem, ErrandCategory } from '../../../../types';
...
export const ShoppingStateMachine: React.FC<Props> = ({ ride, onStatusUpdate }) => {
  ...
  if (currentState === 'SHOPPING') {
    const isShoppingTrip = ride.errandDetails?.category === ErrandCategory.GROCERY_SHOPPING || 
                          ride.errandDetails?.category === ErrandCategory.PERSONAL_SHOPPING;

    if (isShoppingTrip) {
        return (
            <div className="fixed inset-0 z-[9999] bg-[#fbfaff] flex flex-col font-sans">
                <div className="shrink-0 flex items-center bg-[#fbfaff] p-4 pb-2 justify-between border-b border-slate-100 z-10">
                    <button 
                        onClick={() => setCurrentState('ARRIVED_AT_STORE')}
                    ...
                    <h2 className="text-slate-900 text-lg font-bold leading-tight tracking-tight flex-1 ml-2">Shopping Trip</h2>
                    <div className="size-10"></div>
                </div>

                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    <div className="size-24 bg-violet-100 text-violet-600 rounded-full flex items-center justify-center mb-6 animate-pulse">
                        <Icon name="person_pin_circle" className="text-5xl" />
                    </div>
                    <h3 className="text-2xl font-bold text-slate-900 mb-2">Waiting for Customer</h3>
                    <p className="text-slate-500 text-sm mb-8">
                        The customer is currently inside the store doing their shopping. Please stay nearby and wait for them to return to the vehicle.
                    </p>
                    ...
                    <div className="w-full bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
                        <div className="flex items-center gap-4 text-left">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Store</p>
                                <p className="text-sm font-bold text-slate-900">{currentStop.address.split(',')[0]}</p>
                            </div>
                        </div>

                        {items.length > 0 && (
                            <div className="pt-4 border-t border-slate-100 text-left">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Customer Notes</p>
                                <ul className="space-y-2">
                                    {items.map(item => (
                                        <li key={item.id} className="flex items-start gap-2 text-xs text-slate-600">
                                            <Icon name="info" className="text-violet-500 text-sm mt-0.5" />
                                            {item.name}
                                        </li>
                                    ))}
                                </ul>
                ...
                <div className="p-4 bg-white border-t border-slate-200 pb-safe-action">
                    <button 
                        onClick={() => setCurrentState('NAV_TO_DROPOFF')}
                        className="w-full bg-violet-500 hover:bg-violet-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-violet-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3"
                    >
