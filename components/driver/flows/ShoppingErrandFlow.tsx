...
export const ShoppingErrandFlow: React.FC<ErrandFlowProps> = ({ ride, onStatusUpdate }) => {
  ...
  if (localState === 'SHOPPING') {
    ...
    if (isShoppingTrip) {
        return createPortal(
            <div className="fixed inset-0 z-[9999] bg-background-light flex flex-col font-sans h-[100dvh] w-full animate-fade-in overflow-hidden">
                <div className="flex items-center p-4 justify-between bg-white border-b border-slate-200">
                    <div onClick={() => onStatusUpdate(RideStatus.IN_PROGRESS)} className="flex size-10 items-center justify-center rounded-full hover:bg-slate-100 transition-colors cursor-pointer">
                        <Icon name="arrow_back" className="text-slate-700" />
                    </div>
                    <h2 className="text-lg font-bold text-slate-900">Shopping Trip</h2>
                    <div className="size-10"></div>
                </div>

                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    <div className="size-24 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-6 animate-pulse">
                ...
                <div className="p-4 bg-white border-t border-slate-200 pb-safe-action">
                    <button 
                        onClick={() => setLocalState('CHECKOUT')}
                        className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3"
                    >
                    ...
                    <p className="text-center text-slate-400 text-[10px] mt-3 uppercase font-bold tracking-widest">
                        Proceed to payment if you are paying for the goods
                    </p>
                </div>
            </div>,
            document.body
